...
- The **Import entries as** choice exists only on the upload form and applies per import: the form is stateless, **Posts (default)** is preselected every time, and the plugin stores no site-wide setting for the choice. There is no migration or conversion tooling between the two post types — previously imported entries keep the post type they were created with on every rerun (the trash-and-replace rerun described above is the deliberate exception), and the choice affects only entries the current run creates.
- The **Journal Entries** custom post type (`day_one_entry`) is registered only while the plugin is active. Deactivating the plugin unregisters the type and retires its rewrite rules, so Journal Entries permalinks stop resolving while the plugin is inactive; reactivating restores them. Uninstalling removes only plugin options and scheduled events — imported content of **both** post types is retained. While no active plugin registers the `day_one_entry` post type, Journal Entries disappear from wp-admin, but the content itself stays in the database (standard WordPress behavior) and becomes visible again as soon as a plugin registering that post type is active. Imported regular posts are unaffected by deactivation or uninstall.
...
The smoke test also covers the per-import post-type choice. Its import helper can drive a run with either entry post type, and the script verifies: a fresh import as the **Journal Entries** custom post type (`day_one_entry`) with the same fidelity as posts — private status, original entry dates, block content, categories/tags, and location/weather metadata; idempotent reruns in both the same-type and cross-type directions (rerunning with the other choice skips every completed entry and never changes an existing entry's type); trash-and-replace in both directions (exactly one replacement is created, taking the current run's type); a pre-feature job record without a stored post-type choice completing as default posts; in-process private-media scope checks for an attachment whose parent is a Journal Entry (rendered content points at the authenticated private-media endpoint, the import owner can read the parent, a subscriber cannot — full endpoint status-code parity is verified manually instead, see `tests/manual-verification.md`); and the rewrite-rules guard short-circuiting on an ordinary run instead of regenerating stored rewrite rules. The script starts by deleting previously imported content of both post types, so consecutive runs stay green even when an earlier run left Journal Entries behind.
