...
Day One Importer is a WordPress admin importer for Day One JSON exports. It creates one **private** WordPress post per Day One entry and attempts to preserve dates, journal categories, tags, text, supported photos, supported videos, and supported audios.
...
3. The importer expects a ZIP containing one or more journal JSON files with an `entries` array and, when photos, videos, or audios are present, a `photos/`, `videos/`, and/or `audios/` directory.

A typical export follows this shape: a journal JSON file plus media files in `photos/` (and `videos/` / `audios/` when the export contains video or audio moments). Do not commit real Day One exports, photos, videos, or audios to this repository; they can contain private journal content.
...
- Embedded PDF references inside a `richText` payload are still recognized but not yet sideloaded. The unsupported embed logs one privacy-safe warning per entry (no identifier, UUID, or path is included) and produces no block. Full support is tracked as a separate follow-up (PDF #59).
...
Supported initial image types are JPEG/JPG, PNG, and other image formats that the target WordPress site accepts safely. Day One videos (`.mov`, `.mp4`) and Day One audios (`.mp3`, `.m4a`, `.aac`) are imported when the site's MIME allowlist accepts them (the default on most WordPress sites for Administrator-role users); embeds whose MIME the site refuses (most notably `.lpcm` audio on default WordPress allowlists) are dropped with a privacy-safe warning rather than failing the entry. HEIC, PDF, and other attachment types are not guaranteed to import unless WordPress accepts and processes them in that environment.

Photos, videos, and audios are attached to the corresponding private post and importer metadata is stored on the attachment (including the `_day_one_media_kind` marker that distinguishes photo, video, and audio attachments) to support reuse on reruns.
...
- **Photos, videos, or audios missing:** confirm the export includes the relevant `photos/`, `videos/`, and/or `audios/` directory and that the media type is accepted by your site's MIME allowlist (`get_allowed_mime_types()` / the `upload_mimes` filter). Video and audio embeds whose MIME the site refuses (notably `.lpcm` linear-PCM audio on default WordPress allowlists) are dropped with a privacy-safe warning instead of imported as an attachment.
