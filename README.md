...
- When a Day One entry ships a `richText` payload (newer exports), the importer reads it and emits one block per text run, reusing the existing Day One text-to-block conversion. A paragraph run becomes a paragraph block, and a run beginning with `# `, `- `, or `*` still becomes the corresponding markdown-derived heading or list block — matching the way the same content rendered for legacy `text`-only entries. Inline formatting (bold, italic, links, etc.), explicit richText line attributes (proper code blocks, blockquotes, checklists, nested lists, header levels via `attributes.line`), and inline-positioned media are intentionally not interpreted from richText yet and are tracked as separate follow-ups. Legacy entries that ship only the plain `text` field continue to import exactly as before.
...
- Day One rich text fidelity is not guaranteed. The importer reads the `richText` payload when present and emits one block per text run. Inline formatting (bold, italic, links, etc.) and explicit richText line attributes declared via `attributes.line` (proper code blocks, blockquotes, checklists, nested lists, header levels) are not yet interpreted from richText; runs that happen to begin with a markdown sigil (`# `, `- `, `*`) are still rendered through the legacy markdown helper for parity with `text`-only entries. Full richText line-attribute fidelity is tracked as separate follow-ups.
