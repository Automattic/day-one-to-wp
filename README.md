...
- When a Day One entry ships a `richText` payload (newer exports), the importer reads it and emits one block per text run, reusing the existing Day One text-to-block conversion. A paragraph run becomes a paragraph block, and a run beginning with `# `, `- `, or `*` still becomes the corresponding markdown-derived heading or list block — matching the way the same content rendered for legacy `text`-only entries. Supported run-level inline formatting is honored: `bold`, `italic`, `strikethrough`, `inlineCode`, `linkURL` (http(s) only), `autolink` (when paired with a `linkURL`), and `highlightedColor` (Day One's `0xRRGGBB` form) are wrapped in `<strong>`, `<em>`, `<s>`, `<code>`, `<a href="…">`, and `<mark style="background-color:#RRGGBB">` respectively. Non-http(s) links and malformed highlight colors are dropped with a privacy-safe warning (the run's text still renders). Explicit richText line attributes (proper code blocks, blockquotes, checklists, nested lists, header levels via `attributes.line`) and inline-positioned media are still intentionally not interpreted from richText yet and are tracked as separate follow-ups. Legacy entries that ship only the plain `text` field continue to import exactly as before.
...
- Day One rich text fidelity is not guaranteed. The importer reads the `richText` payload when present and emits one block per text run. Run-level inline formatting (bold, italic, strikethrough, inline code, http(s) links, highlight color) is honored; explicit richText line attributes declared via `attributes.line` (proper code blocks, blockquotes, checklists, nested lists, header levels) are not yet interpreted from richText, so runs that happen to begin with a markdown sigil (`# `, `- `, `*`) are still rendered through the legacy markdown helper for parity with `text`-only entries. The highlight `<mark style="background-color:#RRGGBB">` wrapper depends on the site's `wp_kses_post` allowlist accepting `<mark>` with that style attribute; if a host or filter tightens the allowlist, the text still survives and only the highlight color is dropped. Full richText line-attribute fidelity and inline media positioning are tracked as separate follow-ups.
